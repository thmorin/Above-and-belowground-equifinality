      SUBROUTINE soil_eqf(NA,ND,NT,NE,NAX,NDX,NTX,NEX
     2,NHW,NHE,NVN,NVS)
C
...
!-------------------TIM MOD----------------------
      OPEN(871,FILE='param.dat',STATUS='OLD')     
!      READ(871,*)phl_mult,rmyc_mult,DCKI_MULT,TSORP_MULT,SPOHA_MULT
...
!----------NOTE FROM 10/13/2021 - THIS IS THE REAL READER-----
!      READ(871,*)phl_mult,rmyc_mult,DCKI_MULT,OQKM_MULT,SPOHC_MULT
!     2,VCMX_MULT,XKCO2_MULT,ETMX_MULT
      READ(871,*)phl_mult,rmyc_mult,OQKM_MULT,VMXO_MULT,VCMX_MULT
     2,osm_mult,XKCO2_MULT
!      VMXO_MULT=1
!      VCMX_MULT=1
      DCKI_MULT=1
!      OQKM_MULT=1
...
!      XKCO2_MULT=1
      ...
      CLOSE(871)
!      write(*,*)'USING EQUIFINALITY TEST MOD',phl_mult
!-------------------END MOD----------------------
...
C     WRITE(*,333)'STARTS'
      CALL STARTS_EQF(NHW,NHE,NVN,NVS)
C
...
C     WRITE(*,333)'ROUTS'
      CALL ROUTS_EQF(NHW,NHE,NVN,NVS)
      ENDIF
      ENDIF
      ENDIF
C
...
C
C     WRITE(*,333)'READQ'
      CALL READQ_EQF(NA,ND,NT,NE,NAX,NDX,NTX,NEX,NF,NFX,NTZ
     2,NTZX,NHW,NHE,NVN,NVS)
C     WRITE(*,333)'FOUTP'
...
C
C     WRITE(*,333)'NIT'
      CALL NITRO_EQF(I,J,NHW,NHE,NVN,NVS)
C
...
C
C     WRITE(*,333)'GRO'
      CALL GROSUB_EQF(I,J,NHW,NHE,NVN,NVS)
C
