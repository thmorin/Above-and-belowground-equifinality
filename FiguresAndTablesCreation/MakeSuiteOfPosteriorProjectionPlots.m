...
load([dataLoc2 'BootstrappedProjectionW.mat'],...
    ...
    'TRANSPN_proj_med','TRANSPN_proj_95_bds','DTIME');
...
for i=1:length(vList)
    ...
    text(2022,(ylimPrior(2)-ylimPrior(1))*.9+ylimPrior(1),txt1{i},'FontSize',10);
    ...
    box on;
    ylabel({labList{i},['(' unit ')']},'FontSize',10);
    if i==1
        lgd=legend([h1 h3 h_blue h_red],...
            ...
            'Location','NorthOutside','box','off','fontsize',10);
        lgd.NumColumns=2;
    ...
    fprintf('%7s start %% diff,%0.02f\n',var,abs((med_b(1)  -med_10(1))  ./med_b(1  ))*100)
    fprintf('%7s end   %% diff,%0.02f\n'  ,var,abs((med_b(end)-med_10(end))./med_b(end))*100)
