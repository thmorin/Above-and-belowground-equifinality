function [success]=MetropHastings_ecosys(iters,chain_prior,plotYN)

fid_PRM=fopen('PRM_chain.txt','w');
fclose(fid_PRM);

fid_chain=fopen('Chain_status.txt','w');
fclose(fid_chain);

fid_scores=fopen('PRM_scores.txt','w');
fclose(fid_scores);

parNum=4;
postLen=1000;
for i=1:parNum
    copyfile('UMB_base',['run' num2str(i)]);
end
